...
async def test_given_no_todos_expect_empty_list():
    todo_repository_mock = AsyncMock(spec_set=TodoRepository, find=AsyncMock(return_value=[]))
...
async def test_given_todos_expect_list_of_todos(todo: Todo):
    todo_repository_mock = AsyncMock(spec_set=TodoRepository, find=AsyncMock(return_value=[todo]))
